...
export const getPokemonByParams = async ( params ) => {
    ...
    try {
        ...
        return {name: response.data.name, abilities: response.data.abilities, img: [response.data.sprites.front_default,response.data.sprites.back_default, response.data.sprites.front_shiny,response.data.sprites.back_shiny ], type: response1.data.types,  severity:'success', msg:'Datos encontrados correctamente' };

    } catch (error) {
        console.error(error);

        
        return {name: null,  severity:'error', msg:'Datos No encontrados'}
    }
...
export const getListTypePokemon = async ( number = '' ) => {
